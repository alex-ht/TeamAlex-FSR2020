...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc_pitch.sh --cmd "$train_cmd" --nj 24 data/${train_set}_sp || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 24 --cmd "$train_cmd" \
    data/${train_set}_sp data/lang_reestimated $gmm_dir $ali_dir || exit 1
fi
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp ${test_sets}; do
    ...
    #steps/compute_cmvn_stats.sh data/${datadir}_hires_nopitch exp/make_hires/$datadir || exit 1;
  done
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 24 \
    --num-frames 700000 --num-threads 12 \
